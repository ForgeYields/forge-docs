...
**The Atomic Transparency Ledger is the operational counterpart to** [**Hallmark**](../hallmark/overview.md)**.** Where Hallmark publishes the methodology and per-strategy risk scores (_what's allowed in_), the Ledger publishes the real-time state of every allocator action (_what's actually happening_).

Together they form the two pillars of ForgeYields transparency:

| Pillar                            | Surface                                            | Question it answers                           |
| --------------------------------- | -------------------------------------------------- | --------------------------------------------- |
| 🛡 **Hallmark**                   | Per-strategy risk scores, methodology, eligibility | _Why is this strategy in the vault?_          |
| 📜 **Atomic Transparency Ledger** | Live allocator state, every transaction, AUM, PnL  | _What is the vault actually doing right now?_ |
...
| ---------------------------- | ----------------------------------------------------------------------------------------------------- |
| **AUM breakdown**            | Total assets under management, split by chain _and_ by protocol — at the moment of execution          |
